...
def get_stats(img, mask, fn):
    ...
    masks_tot = np.uint8(mask_box_dilate+mask_ellipse_dilate)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7))
    masks_tot = cv2.morphologyEx(masks_tot, cv2.MORPH_OPEN, kernel_erosion,iterations = 5)
    #Apply the closing operation
    edges = (255 - masks_tot)/255
    edges = (1-np.uint8(edges*img+masks_tot)//255)*255
    node_edge = np.uint8(edges + masks_tot)
    ...
    descriptions = open(f'../data/validation/descriptions/'+file +'.txt', 'r')
    ...
    for line in descriptions:
        ...
        list_descriptions.append(first.lower())
        list_descriptions.append(second.lower())
        try:
            ...
            continue
    list_descriptions = list(set(list_descriptions))
    ocr_outputs = list(node_name_dict.values())
    ocr_true = 0
    ocr_total = 0
    ...
    tp_fn = np.where(graph_gt == 1)[0].size
    tp = np.where(graph_gt*graph == 1)[0].size
    tp_fp = np.where(graph == 1)[0].size
    print(tp,tp_fn, tp_fp, ocr_true, ocr_total)
    return tp,tp_fn,tp_fp,ocr_true,ocr_total
